...
static void g_remove_label(struct buttonalt *data){
	gtk_button_set_label(GTK_BUTTON(data->base.widget),NULL);
}

static void g_update_label(struct buttonalt *data){
	gtk_button_set_label(GTK_BUTTON(data->base.widget),data->label);
}

//Requirement: data->label_tf != NULL
static void g_update_label_by_tf(struct buttonalt *data){
	char buffer[200];
...
static void buttonalt_g_on_callback_end(struct buttonalt *data){
	data->callback_id = 0;
}
...
static void on_set_label(struct buttonalt *data){
	if(data->label[0] == '\0'){
		...
		data->callback_id = g_idle_add_full(G_PRIORITY_LOW,G_SOURCE_FUNC(g_remove_label),data,(GDestroyNotify)buttonalt_g_on_callback_end);
	}else if(strchr(data->label,'$') || strchr(data->label,'%')){
		deadbeef->tf_free(data->label_tf);
		data->label_tf = deadbeef->tf_compile(data->label);

		if(data->callback_id != 0) g_source_remove(data->callback_id);
		data->callback_id = g_idle_add_full(G_PRIORITY_LOW,G_SOURCE_FUNC(g_update_label_by_tf),data,(GDestroyNotify)buttonalt_g_on_callback_end);
	}else{
		if(data->callback_id != 0) g_source_remove(data->callback_id);
		data->callback_id = g_idle_add_full(G_PRIORITY_LOW,G_SOURCE_FUNC(g_update_label),data,(GDestroyNotify)buttonalt_g_on_callback_end);
	}
}
...
static void buttonalt_deserialize_from_keyvalues(ddb_gtkui_widget_t *base,const char **keyvalues){
	...
	for(size_t i=0; keyvalues[i]!=NULL; i+=2){
		if(strcmp(keyvalues[i],"action") == 0){
			...
			parse_u32s(&keyvalues[i+1],data->event_updates,sizeof(data->event_updates)/sizeof(data->event_updates[0]),';');
		}
	}
}
...
static void g_option_action_on_changed(GtkEditable* self,gpointer user_data){
	struct buttonalt *data = (struct buttonalt*)user_data;
...
static void g_option_iconname_on_changed(GtkEditable* self,gpointer user_data){
	struct buttonalt *data = (struct buttonalt*)user_data;
...
static void g_option_label_on_changed(GtkEditable* self,gpointer user_data){
	struct buttonalt *data = (struct buttonalt*)user_data;
	const char *s = gtk_entry_get_text(GTK_ENTRY(self));
	if(s){
...
static void g_on_configure_activate(__attribute__((unused)) GtkMenuItem* self,struct buttonalt *data){
		...
		vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL,10);
			...
			hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL,5);
				...
				control = gtk_entry_new();
					...
					g_signal_connect(control,"changed",G_CALLBACK(g_option_action_on_changed),data);
			...
			hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL,5);
				...
				control = gtk_entry_new();
					...
					g_signal_connect(control,"changed",G_CALLBACK(g_option_iconname_on_changed),data);
				gtk_box_pack_start(GTK_BOX(hbox),control,true,true,0);
			gtk_box_pack_start(GTK_BOX(vbox),hbox,false,false,0);
			...
			hbox = gtk_box_new(GTK_ORIENTATION_HORIZONTAL,5);
				...
				control = gtk_entry_new();
					...
					g_signal_connect(control,"changed",G_CALLBACK(g_option_label_on_changed),data);
				gtk_box_pack_start(GTK_BOX(hbox),control,true,true,0);
			gtk_box_pack_start(GTK_BOX(vbox),hbox,false,false,0);
...
static void buttonalt_g_init(struct ddb_gtkui_widget_s *w){
...
static void buttonalt_g_initmenu(struct ddb_gtkui_widget_s *w,GtkWidget *menu){
	GtkWidget *item = gtk_menu_item_new_with_label("Configure");
	g_signal_connect(G_OBJECT(item),"activate",G_CALLBACK(g_on_configure_activate),w);
...
ddb_gtkui_widget_t *buttonalt_create(){
	...
	w->base.init = buttonalt_g_init;
	w->base.initmenu = buttonalt_g_initmenu;
