...
static bool set(const char *label,void *data,void (*skip_change)(void *data)){
	...
	for(size_t i=0; out_plugs[i]; i+=1){
		//If found, set it.
		if(strcmp(out_plugs[i]->plugin.id,label) == 0){
			skip_change(data);
			deadbeef->conf_set_str("output_plugin",label);
...
static bool build(void *combo,int *selected,void (*add)(void *combo,const char *label)){
	...
	deadbeef->conf_lock();
		...
		for(size_t i=0; out_plugs[i]; i+=1){
			add(combo,out_plugs[i]->plugin.id); //TODO: Would be nice to use name also, but set and get is currently based on the combo label
